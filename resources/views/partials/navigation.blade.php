{{-- @dump(request()->route()) --}}
{{-- @dump(request()->routeIs('home')) devuelve true si la ruta actual es home --}}
...
{{-- <nav>
    <ul>
        <Para usar blade se utiliza la sintaxis de dos llaves y dentro se puede usar cualquier función PHP 
        El codigo dentro de las llaves se interpreta como PHP y estara protegido de inyecciones de código 
        Pero si no se quiere esa protección se puede usar la función !! dentro de las llaves para no escaparse 

        <li>
            <a class="{{ request()->routeIs('home') ? 'text-green-600' : 'text-gray-500' }}"
               href="{{ route('home') }}">
                Home
            </a>
        </li>

        <a class="{{ request()->routeIs('posts.*') ? 'text-green-600' : 'text-gray-500' }}"
            href="{{ route('posts.index') }}">
             Blog
         </a>
         El * es para aplicar el estilo en todas las rutas con post....

         <li>
            <a class="{{ request()->routeIs('about') ? 'text-green-600' : 'text-gray-500' }}"
               href="{{ route('about') }}">
                Nosotros
            </a>
        </li>
        ...
        <li>
            <a class="{{ request()->routeIs('contact') ? 'text-green-600' : 'text-gray-500' }}"
               href="{{ route('contact') }}">
                Contacto
            </a>
        </li>

         Con Laravel Breeze instalado, tenemos componentes especiales, como un nav-link el cual se aplicaria asi:
