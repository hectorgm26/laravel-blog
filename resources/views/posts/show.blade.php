...
<x-blog-layout :meta-title="$post->title" :meta-description="$post->body">
    ...
    <article class="mx-auto flex max-w-4xl flex-col">
        ...
        >
          <a
            class="rounded-full bg-sky-600 p-4 text-sky-100 shadow-lg hover:bg-sky-700
             active:bg-sky-800"
            href="{{ route('posts.edit', $post) }}"> {{-- Este boton nos lleva a la ruta posts.edit, que es la ruta para editar el post --}}
            <svg
              class="h-6 w-6"
              data-slot="icon"
              fill="none"
        ...
        <form action="{{ route('posts.destroy', $post) }}" method="POST">
            ...
            @method('DELETE')
            <button type="submit"
            class="rounded-full bg-red-600 p-4 text-red-100 shadow-lg hover:bg-red-700
             active:bg-red-800">
            <svg
              class="h-6 w-6"
              data-slot="icon"
        ...
        <div
          ...
          {{-- Como por el contrador le estamos pasando la variable post, la podemos utilizar --}}
          {{ $post->body }}
        </div>
      </article>
